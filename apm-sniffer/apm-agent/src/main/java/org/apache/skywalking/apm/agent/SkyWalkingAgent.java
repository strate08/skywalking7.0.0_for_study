...
public class SkyWalkingAgent {
    ...
    public static void premain(String agentArgs, Instrumentation instrumentation) throws PluginException {
        ...
        //去忽略一些不需要修改字节码的包 可以理解成设置了aop的切面
        ...
        //使用bytebuddy 修改字节码
        agentBuilder.type(pluginFinder.buildMatch())    //从插件里找到哪一些类需要修改字节码,把匹配规则给找出来
                    .transform(new Transformer(pluginFinder))   //字节码的修改规则
    ...
    private static class Transformer implements AgentBuilder.Transformer {
        ...
        @Override
        public DynamicType.Builder<?> transform(final DynamicType.Builder<?> builder,   //持有被拦截/增强类的字节码
                                                final TypeDescription typeDescription,  //typeDescription 是要被修改class的类
                                                final ClassLoader classLoader,  //负责加载被拦截的类
                                                final JavaModule module) {
            //找出可用于该typeDescription（被拦截类）的所有插件
            List<AbstractClassEnhancePluginDefine> pluginDefines = pluginFinder.find(typeDescription);
            if (pluginDefines.size() > 0) {
                DynamicType.Builder<?> newBuilder = builder;
                //这个EnhanceContext 为了保证流程的一个记录器,比如执行了某个步骤后就会记录一下,防止重复操作

...
public abstract class ClassEnhancePluginDefine extends AbstractClassEnhancePluginDefine {
     ...
     * 仅仅是定义。该方法执行完成后，sw agent知道怎样使用byteBuddy去增强目标类，即增强规则，实际增强是在运行时。
     *
    ...
    private DynamicType.Builder<?> enhanceInstance(TypeDescription typeDescription,
        ...
        //目标类 在执行构造器之前,可能会去执行一些其他的方法,要执行的方法是什么,找出来放入数组,其实就是 aop构造器
        ConstructorInterceptPoint[] constructorInterceptPoints = getConstructorsInterceptPoints();
        //目标类 在执行方法的时候,可能会去执行一些其他的方法,要执行的方法是什么,找出来放入数组
        InstanceMethodsInterceptPoint[] instanceMethodsInterceptPoints = getInstanceMethodsInterceptPoints();
        //目标类 的全路径
        String enhanceOriginClassName = typeDescription.getTypeName();

        //下面都是设置一些标识,用来判断后续是否增强,
        //比如上面的 constructorInterceptPoints和instanceMethodsInterceptPoints 都没有增强方法,那么就直接结束整个方法
         ...
         *
         */
        //1. 向 目标类 写入一个属性 _$EnhancedClassField_ws,类型Object
        //2. 让目标类去实现接口 EnhancedInstance, 实现这个接口的意义在于可以去获取 _$EnhancedClassField_ws 属性,可理解成 getter/setter 了_$EnhancedClassField_ws
         ...
         */
        //增强构造器,也就是把上面 constructorInterceptPoints 中定义好的 增强方法给代理到构造器上面
         ...
         */
        //增强方法 把instanceMethodsInterceptPoints 中定义好的 增强方法给代理到指定方法上面
    ...
    private DynamicType.Builder<?> enhanceClass(TypeDescription typeDescription, DynamicType.Builder<?> newClassBuilder,
        ...
        for (StaticMethodsInterceptPoint staticMethodsInterceptPoint : staticMethodsInterceptPoints) {
            ...
            if (staticMethodsInterceptPoint.isOverrideArgs()) {     //要修改原方法的入参
                if (isBootstrapInstrumentation()) {
                ...
                if (isBootstrapInstrumentation()) {
                    ...
                    //.method() 指定被拦截方法   .intercept() 指定拦截器
                    newClassBuilder = newClassBuilder.method(isStatic().and(staticMethodsInterceptPoint.getMethodsMatcher()))
                                                     .intercept(MethodDelegation.withDefaultConfiguration()
                                                                                .to(new StaticMethodsInter(interceptor)));//interceptor为全类名，需包装
                }
            }
